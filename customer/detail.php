...
<head>
	...
	<meta name="keywords" content="KAOS, WEIST BAG, TOPI, RANSEL" />
	<!-- end: Meta -->
...
<body>
 ...
 <?php
             if(isset($_GET['aksi']) == 'delete'){
				$id = $_GET['kd'];
				$cek = mysqli_query($koneksi, "SELECT * FROM cart WHERE 
					kode='$id' AND session='$_SESSION[user_id]'");
 ...
 <?php
             if(isset($_GET['less']) == 'less'){
				$id_less = $_GET['kd2'];
                $qty1 = 1;
				$cek2 = mysqli_query($koneksi, "SELECT * FROM cart 
					WHERE kode='$id_less' AND session='$_SESSION[user_id]'");
...
<tr>
					<th><center>Purchase Number</center></th>
