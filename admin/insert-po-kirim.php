...
if(isset($_POST['input'])){
				...
				if(mysqli_num_rows($cek) == 0){
						$insert = mysqli_query($koneksi, "INSERT INTO po(nopo, no_resi, tanggalkirim, tanggal_sampai, bukti_kirim, status)
															VALUES('$nopo','', CURRENT_DATE, CURRENT_DATE + interval 3 DAY, '', 'Proses')") or die(mysqli_error());
						if($insert){
							echo "<script>alert('Data PO berhasil disimpan!'); window.location = 'po.php'</script>";
						}else{
