...
//$result=mysqli_query($koneksi, "SELECT * FROM po_terima where id like '%$kodesaya%' ");
    if(isset($_GET['kd'])) {
        $result=mysqli_query($koneksi, "SELECT po_terima.*, produk.nama, harga.harga_jual, 
            customer.no_telp, po.status FROM po_terima
            LEFT JOIN produk ON po_terima.kode = produk.kode
            LEFT JOIN harga ON po_terima.kode = harga.kode
        ...
        if($_GET['awal'] == null && $_GET['akhir'] == null) {
            ...
            $awal = $_GET['awal'];
            $akhir = $_GET['akhir'];
            $result=mysqli_query($koneksi, "SELECT po_terima.*, produk.nama, harga.harga_jual,  customer.no_telp, po.status FROM po_terima
                LEFT JOIN produk ON po_terima.kode = produk.kode
                LEFT JOIN harga ON po_terima.kode = harga.kode
                LEFT JOIN customer ON po_terima.kd_cus = customer.kd_cus
                LEFT JOIN po ON po_terima.nopo = po.nopo
...
//For each row, add the field to the corresponding column
    while($row = mysqli_fetch_array($result)) 
    {
        ...
        $column_nilai = $column_nilai.$harga_jual."\n";
